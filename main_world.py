import argparse
import os
from wam.trainers.train_world import train
...
def parse_args(input_args=None):
    ...
    parser.add_argument(
        ...
        type=str,
        default="/mnt/dataset/projs/projects/RoboTwin/weights/RDT/siglip-so400m-patch14-384",
    ...
    parser.add_argument(
        ...
        type=str,
        default="mixed",
        choices=["mixed", "color_only", "corrput_only", "both"],
        help=(
            "When --image_aug: 'mixed' randomly picks color_only / corrput_only / both each time (default). "
            "'color_only' uses only ColorJitter; 'corrput_only' only image_corrupt; 'both' applies both."
        ),
    )
    parser.add_argument(
        "--precomp_lang_embed",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="预计算语言向量文件名前缀，如 umt5 / qwen3（对应 instructions/umt5.pt）",
    )
    parser.add_argument(
        "--scale_lr",
        action="store_true",
    ...
    parser.add_argument(
        ...
        help=(
            ...
            ' "constant", "constant_with_warmup"], or "custom_cosine" / "custom_constant_then_decay" (see train_world).'
        ),
    )
    parser.add_argument(
        "--lr_hold_ratio",
        ...
        default=0.2,
        help=(
            "For lr_scheduler=custom_constant_then_decay: fraction of total scheduler steps to keep LR constant "
            "(default 0.2), then linear decay to 0."
        ),
    )
    parser.add_argument(
        "--lr_warmup_steps",
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Training-time ``log_sample_res`` 轻量模式：仅 action-only 指标，不做联合 world 推理、不写 mp4。"
            "未指定时：启用 DeepSpeed 则默认为 true。"
        ),
    )
    parser.add_argument(
        "--sample-save-video",
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=False,
        dest="sample_joint_only",
        help="Training-time sampling 只跑 joint/world 生成，不额外跑 action-only 指标。",
    )
    parser.add_argument(
        "--sample-sharded-inference",
        action=argparse.BooleanOptionalAction,
        default=None,
        dest="sample_sharded_inference",
        help=(
            "DeepSpeed 训练期采样：各 rank 同一 batch，经 ZeRO 分片权重集体 predict_action（与训练 forward 一致）。"
            "未指定时：启用 deepspeed 则为 true；EMA 采样时自动 false。"
        ),
    )
    parser.add_argument(
        "--video_sample_guidance",
        type=float,
        default=5.0,
        help=(
            "Video-only training periodic sampling: passed to VideoRunner.conditional_sample as ``guidance``. "
            "Approximately 1.0 disables CFG (single conditional forward); larger values enable two-pass CFG with that scale."
        ),
    )
    parser.add_argument(
        "--debug_viz_steps",
        ...
        default="",
        help=(
            "Comma-separated list of global steps at which to save a debug visualization "
            "comparing GT video (from the current training batch) vs full diffusion sampling. "
            "E.g., '5,10,20'. Empty string disables."
        ),
    )
    parser.add_argument(
        "--mixed_precision",
    ...
    parser.add_argument(
        ...
        default=None,
        required=False,
        help="WandB 等展示用名称；默认在 train 内由 model_config yml 的 CONFIG_NAME 或 yml 文件名推断。",
    )

    parser.add_argument(
        "--pretrained_action_expert_path",
        type=str,
        default="/mnt/dataset/projs/projects/RoboTwin/policy/RDT/checkpoints/robotwin-25000-act/checkpoint-20000",
        help="Path to the pretrained action expert checkpoint.",
    )

    parser.add_argument(
        "--pretrained_video_expert_path",
        type=str,
        default="/mnt/dataset/projs/pretrained_models/WoW-1-Wan-1.3B-2M-Diffusers",
        help="Path to the pretrained video expert checkpoint.",
    )
    ...
    parser.add_argument(
        ...
        default=None,
        required=False,
        help="完整 WorldPolicy（FMPRunner.model）checkpoint 目录；若设置则优先从此加载并忽略分 expert 的微调权重。",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="checkpoints",
        help="The root directory where the checkpoints will be saved.",
    )
